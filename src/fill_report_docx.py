...
def build_body_blocks() -> list[tuple]:
    return [
        ...
        ("h1", "6 实验分析与总结"),
        ("h2", "6.1 结果分析"),
        ("p", "从表 5-1 可以看出，三种模型的性能呈现明显的递进关系：决策树 RMSLE 为 0.2010，随机森林降至 0.1398，LightGBM 进一步降至 0.1243。决策树作为单模型，方差较大且容易在训练集上过拟合，R² 仅为 0.7357；随机森林通过集成多棵树有效降低了方差，R² 提升至 0.8724；LightGBM 采用 Boosting 策略和 leaf-wise 生长，在相同特征工程条件下取得了最优的交叉验证表现。"),
